...
const Rightchat = () => {
  ...
  const updateDimensions = () => {
    if (window.innerWidth < 500) {
      setWindowDimensions({ width: 450, height: 102 });
    } else {
      const update_width = window.innerWidth - 100;
      const update_height = Math.round(update_width / 4.4);
      setWindowDimensions({ width: update_width, height: update_height });
    }
  };
  ...
  return (
    <div
      ...
      className={`right-chat nav-wrap mt-2 right-scroll-bar ${
        windowDimensions.width > 1500 ? 'active-sidebar' : ''
      }`}
    >
            ...
            {chatMember.map((value, index) => (
              <li key={index} className="bg-transparent list-group-item no-icon pe-0 ps-0 pt-2 pb-2 border-0 d-flex align-items-center">
                <figure className="avatar float-left mb-0 me-2">
                  <img src={`assets/images/${value.imageUrl}`} alt="avatar" className="w35" />
                </figure>
